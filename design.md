# Token Monitor 设计拆解：Quota Float

> 参考项目：[change-42-yhmm/quota-float](https://github.com/change-42-yhmm/quota-float)
> 分析日期：2026-07-16
> 目的：提取可迁移到 Token Monitor 的视觉语言、信息架构和交互原则，不直接复制成品。
...
### 对 Token Monitor 的改进建议

Quota Float 的 Aurora 即使在 Orb 状态下仍持续运行。对于目标为“低占用”的 Token Monitor，建议：
...
320×320 的方形构图非常适合一个 provider，但 Token Monitor 需要展示多个 API 余额和多个订阅套餐。如果每个 provider 都使用同尺寸方卡，面板会过长或需要频繁切换。
...
当前视觉主要是浅色柔光。Token Monitor 作为 macOS 菜单栏工具，应支持系统深色模式，但深色模式不能简单地把卡片改成黑色再保留原渐变。
...
1. Token Monitor 默认采用菜单栏 Popover；
...
对 Token Monitor 而言，最合理的方向不是复制它的 320×320 卡片，而是将它的视觉语言扩展成：
