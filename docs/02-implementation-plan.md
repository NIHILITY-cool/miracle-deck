...
Token Monitor 是一个隐私优先、占用低、原生 macOS 的菜单栏工具，用于集中查看：
...
- 已创建 `TokenMonitorCore`、`TokenMonitorProviders` 和
  `TokenMonitorUI` 三个本地 Swift Package；
...
1. 创建远端 GitHub 仓库并配置分支保护；
2. 确定最终项目名称和图标；
3. 确认暂用 Bundle Identifier；
4. 创建 Apple Developer 证书与 notarization 配置。
...
最终命名前必须搜索 GitHub、Homebrew、App Store 和商标数据库，避免与现有产品冲突。
...
```text
token_monitor/
  TokenMonitor.xcodeproj
  App/
    TokenMonitorApp.swift
    AppDelegate.swift
...
```text
~/Library/Caches/app.tokenmonitor.desktop/snapshots/
```
...
```text
service: app.tokenmonitor.desktop.<providerID>
account: <Account UUID>
label: Token Monitor - <Provider> - <Account Name>
```
...
- [ ] 添加代码格式化；
- [x] 添加基础测试；
- [x] 暂定 Bundle ID 为 `app.tokenmonitor.desktop`。
...
2. 创建 `TokenMonitorCore`、`TokenMonitorProviders`、`TokenMonitorUI`；
